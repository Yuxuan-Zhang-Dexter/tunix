#!/bin/bash
# Smoke variant of run_deepswe_disagg_v5p_32.sh, scaled down for v5p-8 (4 chips).
# Goal: verify training stack + r2egym docker backend on TPU host with Qwen3-4B
# in 1-2 train steps. See tasks/tinyflow_integration/track_a.md A.1a.
#
# Usage (from tunix repo root):
#   sg docker -c "bash examples/deepswe/run_deepswe_v5p_8_smoke.sh"
# (sg needed because yuxuan's current shell parent missed the docker group join)
...
checkpoint_dir="${checkpoint_dir:-/mnt/disks/tunix-data/checkpoints/smoke_a1a}"
...
python -m tunix.cli.grpo_main \
  ...
  rl_training_config.metrics_logging_options.log_dir="/tmp/tensorboard/deepswe_smoke_a1a" \
