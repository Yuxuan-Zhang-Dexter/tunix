#!/bin/bash
# A.1b smoke variant — sandbox runs on REMOTE us-west1 N2D VM via DOCKER_HOST=ssh://.
# All other config identical to run_deepswe_v5p_8_smoke.sh.
# See tasks/tinyflow_integration/track_a.md A.1b.
#
# Usage (from tunix repo root):
#   bash examples/deepswe/run_deepswe_v5p_8_smoke_a1b.sh
# (no sg wrap; the local TPU process uses Python's docker SDK over ssh)

set -euo pipefail

export SKIP_JAX_PRECOMPILE=true
# Route r2egym's docker.from_env() to remote N2D rl-swe-test-1 in us-west1-a
export DOCKER_HOST="ssh://yuxuan@10.138.0.7"
...
checkpoint_dir="${checkpoint_dir:-/mnt/disks/tunix-data/checkpoints/smoke_a1b}"
...
python -m tunix.cli.grpo_main \
  ...
  agentic_grpo_config.max_concurrency=2 \
  \
  `# ── GRPO algorithm ───────────────────────────────────────────────────` \
  ...
  rl_training_config.metrics_logging_options.log_dir="/tmp/tensorboard/deepswe_smoke_a1b" \
